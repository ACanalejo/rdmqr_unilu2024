...
- Students must upload their responses to the *reading_questions_responses* folder within the specific session folder (e.g., *02_session02*) in OLAT. 
...
The presentation slides must be uploaded to the specific session folder (e.g., *03_session03*) in OLAT at least one day before the session, that is, the **previous Wednesday, at noon (12:00)**. The  slides can be in PowerPoint or PDF, but the name of the file must always follow the structure *presentation_slides_YOURSURNAME(S)* (e.g., *presentation_slides_CANALEJO*).
